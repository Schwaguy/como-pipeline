 ...
 * @subpackage Como Pipeline/includes
 */
class Como_Pipeline_Sanitize {
	/**
	...
	public function clean() {
		...
		switch ( $this->type ) {
			case 'int'				: $sanitized = (int)($this->data); break;
			case 'color'			:
			case 'radio'			:
			...
			case 'number'			: $sanitized = number_format($this->data, 2); break;
			case 'range'			: $sanitized = intval( $this->data ); break;
			case 'hidden'			:
			case 'month'			:
			//case 'text'				: $sanitized = sanitize_text_field( $this->data ); break;
			case 'text'				: $sanitized = wp_kses_post( $this->data ); break;	
	...
	public function set_type( $type ) {
		$check = '';
		if ( empty( $type ) ) {
			$check = new WP_Error( 'forgot_type', __( 'Specify the data type to sanitize.', 'como-pipeline' ) );
		}
		if ( is_wp_error( $check ) ) {
			wp_die( $check->get_error_message(), __( 'Forgot data type', 'como-pipeline' ) );
		}
		$this->type = $type;
	} // set_type()
} // class